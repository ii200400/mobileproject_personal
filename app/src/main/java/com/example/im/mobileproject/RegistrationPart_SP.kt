...
class RegistrationPart_SP : AppCompatActivity() {
    ...
    var values2 : ContentValues? = null
    var url : Uri? = null
    var url2 : Uri? = null
        ...
        try{
                ...
                override fun onImageAvailable(reader : ImageReader) {
                    ...
                    try{
                        ...
                        if(image != null) {
                            ...
                            url2 = contentResolver.insert(MediaStore.Images.Thumbnails.EXTERNAL_CONTENT_URI, values2)
            ...
            val captureListener : CameraCaptureSession.CaptureCallback = object : CameraCaptureSession.CaptureCallback(){
                override fun onCaptureCompleted(session : CameraCaptureSession, request: CaptureRequest, result : TotalCaptureResult){
                    ...
                    if (url!=null){
                        /*
                        val input : InputStream = this@RegistrationPart_SP.contentResolver.openInputStream(url)
                        val bitmapOptions : BitmapFactory.Options = BitmapFactory.Options()
                        bitmapOptions.inJustDecodeBounds = true
                        bitmapOptions.inPreferredConfig=Bitmap.Config.ARGB_8888
                        val inputBitmap : Bitmap = BitmapFactory.decodeStream(input, null, bitmapOptions)
                        pickedImage.setImageBitmap(inputBitmap)
                        input.close()
                        */
                        val inputBitmap : Bitmap = MediaStore.Images.Media.getBitmap(contentResolver, url)
                        Log.e("----------",inputBitmap.toString())
                        pickedImage.setImageURI(null)
                        pickedImage.setImageURI(url)
                    }
                }
            }
            //매개변수 3개인 함수 불러온 것
